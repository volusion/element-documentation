...
## Add a New Config Shape

Now let's add a more sophisticated structure to our schema. You can group together a set of Element Proptypes using a special type called `shape`. It allows you to nest a set of Element Proptypes inside it.

Back in `config.js`, inside `getConfigSchema`, add this new shape below `isNightModeEnabled`.
...
Update `defaultConfig` to mirror the structure of the schema, but ignoring the `label` and `type`.
...
## Use the `nightModeHours` Shape in the Block

We're going to update the block component so that it compares the current time of day to our new `nightModeHours` to decide what styles to use.

In `block.js`, update the code that's reading the props so that we're getting `nightModeHours`, and also extracting its start and end properties.

```js
const { classes, css, isNightModeEnabled, nightModeHours, text } = props;
const { start, end } = nightModeHours;
```
...
const pmStart = start < 12 ? start + 12 : start;

// is it nighttime in the user's time zone?
const isNight = now >= pmStart || now < end;
...
Changes made to the block config in Site Designer to the `nighModeHours.start` and `nightModeHours.end` fields will not update the block in real time. This is because our `isNightMode` logic above is triggered one time when the block is rendered, but is not tied to a React lifecycle event such as `componentDidUpdate`.
